...
def main_worker(gpu_idx, configs):
    ...
    print(model)
    summary(model, input_size= (3,608,608), batch_size=configs.batch_size)
    ...
    if configs.evaluate:
        ...
        # va_accuracy=accuracy(model,train_dataloader,configs)
        print('val_loss: {:.4e}, accuracy: '.format(val_loss))
        return
    ...
    summary(model, configs.input_size, batch_size=configs.batch_size)
    if tb_writer is not None:
        tb_writer.close()
    if configs.distributed:
        cleanup()
...
def accuracy(network, dataloader ,config):
    network.eval()
    total_correct = 0
    total_instances = 0
    with torch.no_grad():
